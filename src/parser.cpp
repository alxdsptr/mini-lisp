...
ValuePtr Parser::parse(){
    // get the first token from tokens (which type it std::deque<unique_ptr<Token>>)
...
ValuePtr Parser::parseTails(){
    if(tokens.empty()){
        throw SyntaxError("expected content after '('");
    }
    if(tokens.front()->getType() == TokenType::RIGHT_PAREN){
        ...
        if(tokens.front()->getType() == TokenType::DOT){
            ...
            auto cdr = parse();
            if(tokens.empty() or tokens.front()->getType() != TokenType::RIGHT_PAREN){
